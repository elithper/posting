<p align="center">
    <img src="https://github.com/rose-pine/rose-pine-theme/raw/main/assets/icon.png" width="80" />
    <h2 align="center">Rosé Pine for Posting</h2>
</p>

<p align="center">All natural pine, faux fur and a bit of soho vibes for the classy minimalist</p>

<p align="center">
    <a href="https://github.com/rose-pine/rose-pine-theme">
        <img src="https://img.shields.io/badge/community-rosé%20pine-26233a?labelColor=191724&logo=data:image/svg+xml;base64,&style=for-the-badge" />
    </a>
</p>

## Usage

1. Find Posting's themes directory with `posting locate themes`
2. Copy your desired theme(s) to the themes directory
3. (Optional) Preview themes from within Posting by pressing Ctrl+P, selecting `theme: Preview theme`, and choosing your chosen variant from the dropdown
4. In your config file, add the line `theme: rose-pine` (modify for your chosen variant). The path to your config can be found with `posting locate config`.

## Gallery

### Rosé Pine

![Rosé Pine with Posting](/screenshots/rose-pine-posting.png)

### Rosé Pine Moon

![Rosé Pine Moon with Posting](/screenshots/rose-pine-moon-posting.png)

### Rosé Pine Dawn

![Rosé Pine Dawn with Posting](/screenshots/rose-pine-dawn-posting.png)

## Thanks to

- [elithper](https://github.com/elithper)

## Contributing

> Prefer using [@rose-pine/build](https://github.com/rose-pine/build) when possible

Modify `template.yaml` using Rosé Pine variables, then build variants:

```sh
npx @rose-pine/build@latest
```
